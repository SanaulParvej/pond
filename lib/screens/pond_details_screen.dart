...
class _PondDetailsScreenState extends State<PondDetailsScreen> {
  ...
  Future<void> _renamePond() async {
    ...
    if (confirmed == true) {
      ...
      await pondController.renamePond(widget.pondId, name);
      if (mounted) {
        setState(() {});
  ...
  Future<void> _onDownload() async {
    ...
    pw.Widget categorySection(String title, List<Usage> data, {required bool showWeight}) {
      ...
      final rows = data.map((u) {
        ...
        cells.add('Tk ${u.totalPrice.toStringAsFixed(2)}');
        return cells;
      }).toList();
  ...
  Future<void> _showUsageActions(Usage usage) async {
    ...
    final result = await showModalBottomSheet<String>(
      ...
      builder: (sheetContext) {
        ...
        final unitValue = showWeight ? '${usage.weight.toStringAsFixed(2)} ${usage.unit}' : 'Tk ${usage.totalPrice.toStringAsFixed(2)}';
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (unitPrice != null)
                  _DetailInfoRow(label: 'Unit Price', value: 'Tk ${unitPrice.toStringAsFixed(2)}'),
                _DetailInfoRow(label: 'Total Cost', value: 'Tk ${usage.totalPrice.toStringAsFixed(2)}', emphasize: true),
                const SizedBox(height: 20),
                FilledButton.icon(
  ...
  Future<void> _editUsageEntry(Usage usage, String categoryKey) async {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      for (final usage in usages) {
        final key = usage.date.toLocal().toIso8601String().split('T').first;
        grouped.putIfAbsent(key, () => <Usage>[]).add(usage);
      }
                  ...
                  children: [
                    ...
                    if (usages.isEmpty)
                      ...
                      ...sortedDates.expand((dateKey) {
                        final items = grouped[dateKey] ?? <Usage>[];
                        DateTime? parsedDate;
                        try {
                          parsedDate = DateTime.tryParse(dateKey)?.toLocal();
                        } catch (_) {
                          parsedDate = null;
                        }
                        final dateHeading = parsedDate != null ? _formatDisplayDate(parsedDate) : dateKey;
                        return [
                          Padding(
                            padding: const EdgeInsets.symmetric(vertical: 8.0),
                            child: Text(dateHeading, style: Theme.of(context).textTheme.titleMedium),
                          ),
                          ...items.map((u) {
                            final categoryKey = _categoryForUsage(u);
                            final accent = _categoryColor(categoryKey, Theme.of(context));
                            final showWeight = categoryKey != 'other';
                            final hasWeight = showWeight && u.weight > 0;
                            final weightLabel = hasWeight ? '${u.weight.toStringAsFixed(2)} ${u.unit}' : null;
                            final unitPrice = hasWeight ? u.totalPrice / u.weight : null;
                            ...
                            return Padding(
                              ...
                              child: Card(
                                ...
                                shape: RoundedRectangleBorder(
                                  ...
                                  side: BorderSide(color: accent.withAlpha(40)),
                                ),
                                child: InkWell(
                                  onTap: () => _showUsageActions(u),
                                  borderRadius: BorderRadius.circular(12),
                                  child: Padding(
                                        ...
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              if (unitPrice != null)
                                                Padding(
                                                  padding: EdgeInsets.only(top: weightLabel == null ? 4 : 2),
                                                  child: Text(
                                                    'Unit price: Tk ${unitPrice.toStringAsFixed(2)}',
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('Tk ${totalCost.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w700)),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 12),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('Tk ${usage.totalPrice.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w600)),
            const SizedBox(height: 6),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Text('Tk ${amount.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w700)),
      ]),
    );
  }
}
