...
class _AddUsageScreenState extends State<AddUsageScreen> {
  ...
  bool get isEditing => widget.initialUsage != null && widget.usageId != null;
    ...
    if (_isOtherMode) {
      ...
      if (usage.weight > 0) {
        ...
        _priceCtrl.text =
            ...
            usage.totalPrice.toStringAsFixed(2);
      }
    }
  }
  ...
  void _save() async {
    ...
    if (isOther) {
      ...
      final usage = Usage(
        id: widget.usageId ?? widget.initialUsage?.id,
        pondId: pondId,
        date: _selectedDate,
        productCode: 'OTHER',
        productName: name,
      ...
      final success = isEditing
          ? await pondController.updateUsage(usage)
          : await pondController.addUsage(usage);
      if (success) {
        Get.back(result: true);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Unable to save expense right now')),
        );
      }
      return;
    }
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Unable to save usage right now')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    List<Product> available = pondController.products.toList(growable: false);
    if (_categoryFilter != null && !isOther) {
      final t = _categoryFilter!;
      available = pondController.products
          .where((p) => p.category.toLowerCase() == t)
          .toList();
    }
    ...
    final pondOptions = List.generate(
      pondController.pondCount,
      (i) => DropdownMenuItem(
        value: i + 1,
        child: Text(pondController.pondName(i + 1)),
      ),
    );
    return Scaffold(
      appBar: AppBar(
        title: Text(isEditing ? 'Edit Usage' : 'Add Product Usage'),
      ),
                  ...
                  children: [
                    ...
                    if (!isOther) ...[
                      TextField(
                        ...
                        readOnly: true,
                        decoration: const InputDecoration(
                          labelText: 'Selected Product Code',
                        ),
                      ),
                      const SizedBox(height: 12),
                      Container(
                                ...
                                children: [
                                  ...
                                  Text(
                                    _unitCtrl.text.isEmpty
                                        ? ''
                                        : '${_unitCtrl.text} • Tk ${_priceCtrl.text}',
                                  ),
                                ],
                              ),
                            ),
